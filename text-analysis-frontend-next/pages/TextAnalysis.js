...
import BarGraph from "@/components/vis/BarGraph";
import BarChartD3 from "@/components/vis/BarChartD3";
...
const TextAnalysis = () => {
  ...
  const containerStyle = { overflowWrap: "break-word", wordWrap: "break-word", whiteSpace: "normal" };
  ...
  const loadMetaText = async () => {
    ...
    try {
      ...
      // To make the url matches plans, we remove the details of current page (TextAnalysis) from query
      const strAnalyze = is_localhost
        ? window.location.href.replace("TextAnalysis",'').replace("3000", "5000") + "api/analyze"
        : window.location.href.replace("TextAnalysis",'') + "api/analyze";
  ...
  const mapLengthToHue = (length, minLength, maxLength) => {const range = maxLength - minLength;const scale = length - minLength;return 240 - (scale / range) * 240;};
  const getBackgroundColor = (length) => {
    const hue = mapLengthToHue(length, minLength, maxLength);const saturation = 70;const lightness = 50;const alpha = 0.85; return `hsla(${hue}, ${saturation}%, ${lightness}%, ${alpha})`;
  };
  const getBackgroundColorAI = (outputAI, index) => {
    ...
    const hue = mapLengthToHue(curScore, minScore, maxScore); const saturation = 70; const lightness = 50; const alpha = 0.85;
  ...
  function processText(text) {
    ...
    const styledWords = words.map((word, index) => {
      ...
      const style = { backgroundColor, color: "#fff", padding: "2px 4px", margin: "2px", display: "inline-block", border: indicesWithBorder.has(index) ? "2px solid red" : "none", };
  ...
  const handleTextChange = (event) => {setPrevText(text);setMetaText("");setProgress(0);setText(event.target.value);};
  ...
  return (
    <>
        ...
        <div className="container-fluid d-flex flex-column min-vh-100">
          ...
          <div className="row">
            ...
            <div className="col-md-10">
              {/* <h1 className="heading-section text-center">Text Analysis</h1> */}
              <ProductFeatureTitle feature="Text Analysis"/>
      ...
      <div className="textSearchOutput">
        ...
        {/* <div className="row">
          {progress === 100 && (
            <><h3 className="heading-section text-center">Score Graphs</h3><div style={{ width: "100%", height: "400px" }}><BarChartD3 data={processedData} /></div><BarGraph data={graphData} /></>
          )}
        </div> */}
      </div>
...
export default withAuth(TextAnalysis);