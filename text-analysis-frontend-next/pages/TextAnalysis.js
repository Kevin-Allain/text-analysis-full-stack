  ...
  function processText(text) {
    ...
    const styledWords = words.map((word, index) => {
      ...
      const style = {
        ...
        border: indicesWithBorder.has(index) ? "2px solid red" : "none",
      };

      return `<span style="${Object.entries(style)
        .map(([k, v]) => `${k}:${v}`)
        .join(";")}">${word}</span>`;
    });
    ...
    const data = [
      {
        ...
        description:
          "Originality is assessed with likeliness of words used to be often present in text if generated by AI.",
      },
      {
        name: "Complexity",
        value: 0.3 + Math.random() * 0.6,
        description:
          "Complexity recognizes the likeliness of sentence complexity matching what is often found in AI generated texts.",
      },
      {
        name: "Metadata",
        value: 0.4 + Math.random() * 0.3,
        description:
          "Metadata analysis interprets if the information associated to your document appears suspicious.",
      },
      {
        name: "Tables/Images",
        value: 0.2 + Math.random() * 0.3,
        description:
          "Images and tables from the document uploaded can indicate if content was generated by AI.",
      },
      {
        name: "Footnotes",
        value: 0.3 + Math.random() * 0.4,
        description:
          "Footnotes of documents can contain traces often encountered with text generated by AI..",
      },
    ];
    const avgvalue =
      data.map((a) => a.value).reduce((a, b) => a + b) / data.length;
    data.push({
      name: "Average score",
      value: avgvalue,
      description:
        "This score is an average. We recommend looking at the detail for a stronger understanding.",
    });
    return data;
  };
